...
class YOLO(object):
    ...
    def detect(self, direction, save_img=False):
        #  基本参数配置
        parser = argparse.ArgumentParser()
        parser.add_argument('--weights', nargs='+', type=str,
                            default='Pre_warning_system/Scenic_spot_safety_waring_yolov5/weights/best.pt',
                            help='model.pt path(s)')
        parser.add_argument('--source', type=str, default=direction, help='source')  # file/folder, 0 for webcam
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if view_img:
                    ...
                    for c in det[:, -1].unique():
                        n = (det[:, -1] == c).sum()
                        self.all_per += n
                    cv2.putText(im0, f"persons: {self.all_per} ", (5, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 2)
                ...
                if save_img:
                    ...
                    #  结果保存的路径
                    self.res_dir = str(save_path)
                    print("结果相对路径：" + self.res_dir)
